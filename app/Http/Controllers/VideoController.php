...
class VideoController extends Controller
{
    ...
    public function OpenContent(Request $request, $id)
    {
        ...
        $videoList =  DB::table("contents")->select("id", "title", 'user_id', 'path', 'thumbnail', 'duration')->where('user_id', '=', auth()->id())->get();

        $comments = DB::table("comments")->select('user_id', 'comment_text', 'created_at')->where('content_id', '=', $id)->get();
    ...
    public function unsaveVideo(Request $request)
    {
        ...
        DB::table("likes")->where("content_id", "=", $info["content_id"])->where('user_id', '=', auth()->id())->delete();
        return back();
    }

    public function checkIfSaved($id)
    {
        $saved = DB::table("likes")->select('user_id', 'content_id')->where("content_id", "=", $id)->get();

        $savedByUser = DB::table("likes")->select('user_id', 'content_id')->where("content_id", "=", $id)->where('user_id', '=', auth()->id())->get();
